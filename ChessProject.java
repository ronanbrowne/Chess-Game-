...
public class ChessProject extends JFrame implements MouseListener, MouseMotionListener {
    ...
    Boolean agentwins;
    Stack temporary;
    int optionPicked;// Picking a move
    ...
    //gets the value of the piece & location on the board
    public int pieceValue(int x, int y, String pieceName){
      int value = 0;
      String pieceColor = "";

      if(pieceName.contains("White")){//gives value of oponents colour
        pieceColor = "Black";
      }else{
        pieceColor = "White";
      }
	...
	// Setting up the Initial Chess board.
	  //deleted comments, brought Pawn to the board to play!!
  	for(int i=8;i < 16; i++){
...
private Stack getPawnMoves(int x, int y, String piece){
  //takes a piece and checks if it can move
  Square startingSquare = new Square(x, y, piece);
  Stack moves = new Stack();
  Move validM;
  int tmpx1 = x+1;//to the right move
  int tmpx2 = x-1;// to the left move
  int tmpy1= y+1;// forward(down) 1 move
  int tmpy2 = y+2;// forward(down) 2 move

  //to be completed
  //below tmps equal the possible moves the white awn can take.

  Square tmp4 = new Square(x, tmpy2, piece);//if starting piece
  Square tmp3 = new Square(x, tmpy1, piece);//does not take a piece
  Square tmp = new Square(tmpx1, tmpy1, piece); //takes a piece to the south east of board
  Square tmp2 = new Square(tmpx2, tmpy1, piece);//takes a piece to the south west of board

//starting position
if(y == 1 && !piecePresent(((tmp3.getXC()*75)+20),((tmp3.getYC()*75)+20))){//this makes sure there isnt a piece in front of the pawn
  //so basically on the first move this stops it jumping over a piece


  validM = new Move(startingSquare, tmp4, pieceValue(((tmp4.getXC()*75)+20), ((tmp4.getYC()*75)+20),piece));
  //preparing the move to be sent in
    if((!piecePresent(((tmp4.getXC()*75)+20), ((tmp4.getYC()*75)+20)))){
      moves.push(validM);
    }
    //this checks if there isnt a piece present the move is valid.
}

 if((tmpx1 <= 7 || tmpx2 >= 0)&&(tmpy1 <= 7)){//not starting position... stops pieces moving outside board.


      validM = new Move(startingSquare, tmp, pieceValue(((tmp.getXC()*75)+20), ((tmp.getYC()*75)+20),piece) );//only if a piece can be taken
        if(checkWhiteOponent(((tmp.getXC()*75)+20), (((tmp.getYC()*75)+20)))){
          moves.push(validM);
        }
        //checking to see if the piece is black, then pushes the move if it is(adds to stack)

      validM = new Move(startingSquare, tmp2,pieceValue(((tmp2.getXC()*75)+20), ((tmp2.getYC()*75)+20),piece));//only if a piece can be taken
        if(checkWhiteOponent(((tmp2.getXC()*75)+20), (((tmp2.getYC()*75)+20)))){
          moves.push(validM);
        }//same as above

      validM = new Move(startingSquare, tmp3, pieceValue(((tmp3.getXC()*75)+20), ((tmp3.getYC()*75)+20),piece));
        if((!piecePresent(((tmp3.getXC()*75)+20), ((tmp3.getYC()*75)+20)))){
          moves.push(validM);
        }//this is just for a forward move. makes sure there isnt a piece in front.
  }

  return moves; //retuns the Stack of Moves called Moves.
  //contains all possible white Pawn moves
}
...
private Boolean checkSurroundingSquares(Square s){
  ...
  return possible;//checks squares around the king
 }
...
private Stack getKingMoves(int x, int y, String piece){//returns all possible white moves

  //used white pawn logic here
  //very much the same
  Square startingSquare = new Square(x, y, piece);
  Stack moves = new Stack();
...
*/
  ...
  if(!(tmpx1 > 7)){
    ...
    */
    ...
    if(checkSurroundingSquares(tmp)){
      validM = new Move(startingSquare, tmp, pieceValue(((tmp.getXC()*75)+20), ((tmp.getYC()*75)+20),piece));
    ...
    if(!(tmpy1 > 7)){
      if(checkSurroundingSquares(tmp1)){
        validM2 = new Move(startingSquare, tmp1,pieceValue(((tmp1.getXC()*75)+20), ((tmp1.getYC()*75)+20),piece));
    ...
    if(!(tmpy2 < 0)){
      if(checkSurroundingSquares(tmp2)){
        validM3 = new Move(startingSquare, tmp2,pieceValue(((tmp2.getXC()*75)+20), ((tmp2.getYC()*75)+20),piece));
  ...
  if(!((tmpx2 < 0))){
    ...
    if(!(tmpy1 > 7)){
      if(checkSurroundingSquares(tmp4)){
        validM2 = new Move(startingSquare, tmp4,pieceValue(((tmp4.getXC()*75)+20), ((tmp4.getYC()*75)+20),piece));
        if(!piecePresent(((tmp4.getXC()*75)+20), (((tmp4.getYC()*75)+20)))){
          moves.push(validM2);
        }
        else{
    ...
    if(!(tmpy2 < 0)){
      if(checkSurroundingSquares(tmp5)){
        validM3 = new Move(startingSquare, tmp5,pieceValue(((tmp5.getXC()*75)+20), ((tmp5.getYC()*75)+20),piece));
  ...
  if(!(tmpy1 > 7)){
    if(checkSurroundingSquares(tmp7)){
      validM2 = new Move(startingSquare, tmp7,pieceValue(((tmp7.getXC()*75)+20), ((tmp7.getYC()*75)+20),piece));
  ...
  if(!(tmpy2 < 0)){
    if(checkSurroundingSquares(tmp8)){
      validM3 = new Move(startingSquare, tmp8,pieceValue(((tmp8.getXC()*75)+20), ((tmp8.getYC()*75)+20),piece));
  ...
  return moves;//retuns all possible king moves,get king squares
}
  ...
  */
  ...
  for(int i=1;i < 8;i++){
    ...
    if(!(tmpx > 7 || tmpx < 0)){
      ...
      if(!piecePresent(((tmp.getXC()*75)+20), (((tmp.getYC()*75)+20)))){
        ...
        if(checkWhiteOponent(((tmp.getXC()*75)+20), ((tmp.getYC()*75)+20))){
          moves.push(validM);
          break;//if piece found, loop stops, goes for first piece found
        }
        else{
          break;
        }
  ...
  for(int j=1;j < 8;j++){
    ...
    if(!(tmpx1 > 7 || tmpx1 < 0)){
      ...
      validM2 = new Move(startingSquare, tmp2,pieceValue(((tmp2.getXC()*75)+20), ((tmp2.getYC()*75)+20),piece));
      if(!piecePresent(((tmp2.getXC()*75)+20), (((tmp2.getYC()*75)+20)))){
        moves.push(validM2);
      }
      else{
  ...
  for(int k=1;k < 8;k++){
    ...
    if(!(tmpy3 > 7 || tmpy3 < 0)){
      ...
      validM3 = new Move(startingSquare, tmp3,pieceValue(((tmp3.getXC()*75)+20), ((tmp3.getYC()*75)+20),piece) );
      if(!piecePresent(((tmp3.getXC()*75)+20), (((tmp3.getYC()*75)+20)))){
        moves.push(validM3);
      }
      else{
  ...
  for(int l=1;l < 8;l++){
    ...
    if(!(tmpy4 > 7 || tmpy4 < 0)){
      ...
      validM4 = new Move(startingSquare, tmp4,pieceValue(((tmp4.getXC()*75)+20), ((tmp4.getYC()*75)+20),piece));
      if(!piecePresent(((tmp4.getXC()*75)+20), (((tmp4.getYC()*75)+20)))){
        moves.push(validM4);
      }
      else{
  ...
  */
  ...
  for(int k=1;k < 8;k++){
    ...
    if(!(tmpk > 7 || tmpk < 0 || tmpy2 > 7 || tmpy2 < 0)){
      ...
      validM2 = new Move(startingSquare, tmpK1,pieceValue(((tmpK1.getXC()*75)+20), ((tmpK1.getYC()*75)+20),piece));
  ...
  for(int l=1;l < 8;l++){
    ...
    if(!(tmpL2 > 7 || tmpL2 < 0 || tmpy3 > 7 || tmpy3 < 0)){
      ...
      validM3 = new Move(startingSquare, tmpLMov2,pieceValue(((tmpLMov2.getXC()*75)+20), ((tmpLMov2.getYC()*75)+20),piece));
  ...
  for(int n=1;n < 8;n++){
    ...
    if(!(tmpN2 > 7 || tmpN2 < 0 || tmpy4 > 7 || tmpy4 < 0)){
      ...
      validM4 = new Move(startingSquare, tmpNmov2,pieceValue(((tmpNmov2.getXC()*75)+20), ((tmpNmov2.getYC()*75)+20),piece));
...
//this checks to see if the piece is still there

public boolean Move2PieceP(int bx,int by,int wox,int woy){ //this checks to see if the piece is still there
  boolean PiecePresent = false;
  if(piecePresent(bx,by)){
    if(wox!=bx && woy!=by){
    PiecePresent=true;
  }
}
...
private int getBlckPawnPoints(int x, int y, String piece, int nwx, int nwy, int owx, int owy){



  Square startingSqueare = new Square(x,y, piece);
  ...
  Square tmp3 = new Square(tmpx1, tmpy1, piece); //takes a piece to the north east of board
  Square tmp4 = new Square(tmpx2, tmpy1, piece);//takes a piece to the north west of board

    if((tmpx1 <= 7)&&(tmpy1 >= 0)){
    //this checks if the piece is going to be taken by a pawn

      if((nwx== (tmp3.getXC())) && (nwy == (tmp3.getYC()))){
        value = pieceValue(((owx*75)+20),((owy*75)+20), piece);
        values.push(value);
        //old position of piece going to be taken (where the piece is actually placed)?
      }
      //checks if it can take another piece already there with in mind the piece may not be present if the old xy equals the position
      else if(Move2PieceP(((tmp3.getXC()*75)+20),((tmp3.getYC()*75)+20),owx,owy)){
        value = pieceValue(((tmp3.getXC()*75)+20), ((tmp3.getYC()*75)+20), piece);
        values.push(value);
      }
    }
    ...
    if((tmpx2 >= 0)&&(tmpy1 >= 0)){
          ...
          if((nwx== (tmp4.getXC())) && (nwy == (tmp4.getYC()))){
          value = pieceValue(((owx*75)+20), ((owy*75)+20), piece);
          values.push(value);
        }
        else if(Move2PieceP(((tmp4.getXC()*75)+20),((tmp4.getYC()*75)+20),owx,owy)){
          value = pieceValue(((tmp4.getXC()*75)+20), ((tmp4.getYC()*75)+20), piece);
          values.push(value);
        }
      }
      int HighestValue=0;
...
private int getBlackKingPoints(int x, int y, String piece, int nwx, int nwy, int owx, int owy){
  ...
  if(!(tmpx1 > 7)){
    ...
    if(checkSurroundingSquares(tmp)){
      if((nwx== (tmp.getXC())) && (nwy == (tmp.getYC()))){
        value = pieceValue(((owx*75)+20),((owy*75)+20), piece);
        values.push(value);
        //old position of piece going to be taken (where the piece is actually placed)?
      }
      else if(Move2PieceP(((tmp.getXC()*75)+20),((tmp.getYC()*75)+20),owx,owy)){
        value = pieceValue(((tmp.getXC()*75)+20), ((tmp.getYC()*75)+20), piece);
        values.push(value);
      }
    }
    ...
    if(!(tmpy1 > 7)){
      if(checkSurroundingSquares(tmp1)){
        if((nwx== (tmp1.getXC())) && (nwy == (tmp1.getYC()))){
          value = pieceValue(((owx*75)+20),((owy*75)+20), piece);
          values.push(value);
          //old position of piece going to be taken (where the piece is actually placed)?
        }
        else if(Move2PieceP(((tmp1.getXC()*75)+20),((tmp1.getYC()*75)+20),owx,owy)){
          value = pieceValue(((tmp1.getXC()*75)+20), ((tmp1.getYC()*75)+20), piece);
          values.push(value);
        }
        }
      }
    ...
    if(!(tmpy2 < 0)){
      if(checkSurroundingSquares(tmp2)){
        if((nwx== (tmp2.getXC())) && (nwy == (tmp2.getYC()))){
          value = pieceValue(((owx*75)+20),((owy*75)+20), piece);
          values.push(value);
          //old position of piece going to be taken (where the piece is actually placed)?
        }
        else if(Move2PieceP(((tmp2.getXC()*75)+20),((tmp2.getYC()*75)+20),owx,owy)){
          value = pieceValue(((tmp2.getXC()*75)+20), ((tmp2.getYC()*75)+20), piece);
          values.push(value);
        }
      }
    }
  ...
  if(!((tmpx2 < 0))){
    ...
    if(!(tmpy2 < 0)){
      if(checkSurroundingSquares(tmp5)){
        if((nwx== (tmp5.getXC())) && (nwy == (tmp5.getYC()))){
          value = pieceValue(((owx*75)+20),((owy*75)+20), piece);
          values.push(value);
          //old position of piece going to be taken (where the piece is actually placed)?
        }
        else if(Move2PieceP(((tmp5.getXC()*75)+20),((tmp5.getYC()*75)+20),owx,owy)){
          value = pieceValue(((tmp5.getXC()*75)+20), ((tmp5.getYC()*75)+20), piece);
          values.push(value);
        }
        }
      }
  ...
  if(!(tmpy1 > 7)){
    if(checkSurroundingSquares(tmp7)){
      if((nwx== (tmp7.getXC())) && (nwy == (tmp7.getYC()))){
        value = pieceValue(((owx*75)+20),((owy*75)+20), piece);
        values.push(value);
        //old position of piece going to be taken (where the piece is actually placed)?
      }
      else if(Move2PieceP(((tmp7.getXC()*75)+20),((tmp7.getYC()*75)+20),owx,owy)){
        value = pieceValue(((tmp7.getXC()*75)+20), ((tmp7.getYC()*75)+20), piece);
        values.push(value);
      }
    }
  }
  ...
  if(!(tmpy2 < 0)){
    if(checkSurroundingSquares(tmp8)){
      if((nwx== (tmp8.getXC())) && (nwy == (tmp8.getYC()))){
        value = pieceValue(((owx*75)+20),((owy*75)+20), piece);
        values.push(value);
        //old position of piece going to be taken (where the piece is actually placed)?
      }
      else if(Move2PieceP(((tmp8.getXC()*75)+20),((tmp8.getYC()*75)+20),owx,owy)){
        value = pieceValue(((tmp8.getXC()*75)+20), ((tmp8.getYC()*75)+20), piece);
        values.push(value);
      }
    }
  }
...
private int getQueenPoints(int x, int y, String piece, int nwx, int nwy, int owx, int owy){
  int RookPoints = 0;
  int BishopPoints = 0;
  RookPoints = getRookPoints(x, y, piece,nwx,nwy,owx, owy);
  BishopPoints= getBishopPoints(x, y, piece,  nwx, nwy, owx, owy);

  if(RookPoints>BishopPoints){
    return RookPoints;
  }else{
    return BishopPoints;
  }
...
private int getRookPoints(int x,int  y,String piece, int nwx,int nwy, int owx, int owy){//
  Square startingSquare = new Square(x, y, piece);
  Stack values = new Stack();
  int value;
  Move validM, validM2, validM3, validM4;
...
private int getBishopPoints(int x,int  y,String piece, int nwx,int nwy, int owx, int owy){
  Square startingSquare = new Square(x, y, piece);
  Stack values = new Stack();
  int value;
  Move validM, validM2, validM3, validM4;
  ...
  for(int k=1;k < 8;k++){
    ...
    if(!(tmpk > 7 || tmpk < 0 || tmpy2 > 7 || tmpy2 < 0)){
      ...
      if((nwx== (tmpK1.getXC())) && (nwy == (tmpK1.getYC()))){
        value = pieceValue(((owx*75)+20),((owy*75)+20), piece);
        values.push(value);
        break;
        //old position of piece going to be taken (where the piece is actually placed)?
      ...
      else if(Move2PieceP(((tmpK1.getXC()*75)+20),((tmpK1.getYC()*75)+20),owx,owy)){
        value = pieceValue(((tmpK1.getXC()*75)+20), ((tmpK1.getYC()*75)+20), piece);
        values.push(value);
        break;
      }
    }
  ...
  for(int l=1;l < 8;l++){
    ...
    if(!(tmpL2 > 7 || tmpL2 < 0 || tmpy3 > 7 || tmpy3 < 0)){
      Square tmpLMov2 = new Square(tmpL2, tmpy3, piece);
      if((nwx== (tmpLMov2.getXC())) && (nwy == (tmpLMov2.getYC()))){
        value = pieceValue(((owx*75)+20),((owy*75)+20), piece);
        values.push(value);
        break;
        //old position of piece going to be taken (where the piece is actually placed)?
      ...
      else if(Move2PieceP(((tmpLMov2.getXC()*75)+20),((tmpLMov2.getYC()*75)+20),owx,owy)){
        value = pieceValue(((tmpLMov2.getXC()*75)+20), ((tmpLMov2.getYC()*75)+20), piece);
        values.push(value);
        break;
      }
    }
  ...
  for(int n=1;n < 8;n++){
    ...
    if(!(tmpN2 > 7 || tmpN2 < 0 || tmpy4 > 7 || tmpy4 < 0)){
      Square tmpNmov2 = new Square(tmpN2, tmpy4, piece);
      if((nwx== (tmpNmov2.getXC())) && (nwy == (tmpNmov2.getYC()))){
        value = pieceValue(((owx*75)+20),((owy*75)+20), piece);
        values.push(value);
        break;
        //old position of piece going to be taken (where the piece is actually placed)?
      ...
      else if(Move2PieceP(((tmpNmov2.getXC()*75)+20),((tmpNmov2.getYC()*75)+20),owx,owy)){
        value = pieceValue(((tmpNmov2.getXC()*75)+20), ((tmpNmov2.getYC()*75)+20), piece);
        values.push(value);
        break;
      }
    }
...
/*
    ...
    board as long as the landing square is on the board and we can take an opponents piece but not our own piece.
*/
private int getKnightPoints(int x,int  y,String piece, int nwx,int nwy, int owx, int owy){
  Square startingSquare = new Square(x, y, piece);
  Stack moves = new Stack();
  Stack values = new Stack();
  int value;
  ...
  for(int i=0;i < 8;i++){
    ...
    if((tmp.getXC() < 0)||(tmp.getXC() > 7)||(tmp.getYC() < 0)||(tmp.getYC() > 7)){
      ...
      if((nwx== ((tmp.getXC()*75)+20)) && (nwy == ((tmp.getYC()*75)+20))){
        value = pieceValue(((owx*75)+20),((owy*75)+20),piece);
        values.push(value);//old position of piece going to be taken (where the piece is actually placed)
      }
...
private Stack getKnightMoves(int x, int y, String piece){
  ...
  for(int i=0;i < 8;i++){
    Square tmp = (Square)moves.pop();
    Move tmpmove = new Move(startingSquare, tmp,pieceValue(((tmp.getXC()*75)+20), ((tmp.getYC()*75)+20),piece) );
    if((tmp.getXC() < 0)||(tmp.getXC() > 7)||(tmp.getYC() < 0)||(tmp.getYC() > 7)){
...
*/
  ...
  private ArrayList<Square> findBlackPieces(){
...
private void printStack(Stack input){
  ...
  private void makeAIMove(){
    ...
    */
    ...
    ArrayList<Square> black = null;//gets all the black pieces locations
    if(optionPicked >= 2){
      black = findBlackPieces();
      }
    Stack completeMoves = new Stack();
    Move tmp;
    while(!white.empty()){
...
*/
  ...
  System.exit(0);
    ...
    */
      ...
      if(optionPicked<2){
        selectedMove = agent.MethodPicked(optionPicked,completeMoves);
      }else{

        Stack values = new Stack();
        Stack highVales = new Stack();
        Stack whiteMoves = new Stack();

        while(!completeMoves.empty()){//loops through all possible white moves
          int tempHigh = -1;
          Move Moves = (Move)completeMoves.pop();
          Square currentWhite = (Square)Moves.getStart();
          Square newWhite = (Square)Moves.getLanding();

          for(int qu = 0; qu < black.size();qu++){
//loops through all the black pieces to find out which move has the highest return value
            int value= 0;
            Square blackpiece = (Square)black.get(qu);
            String name = (String)blackpiece.getName();

            if((newWhite.getXC() != blackpiece.getXC())&&(newWhite.getYC() != blackpiece.getYC())){
              //above if makes sure the black pices isnt taken by the white move.
              if(name.contains("Pawn")){
                value = getBlckPawnPoints(blackpiece.getXC(), blackpiece.getYC(), blackpiece.getName(), newWhite.getXC(), newWhite.getYC(), currentWhite.getXC(), currentWhite.getYC());
          //all these return the highest value black can take with a piece in this case a pawn
                values.push(value);//pushes the highest pawn value to a stack
              }

              else if(name.contains("Rook")){
                value = getRookPoints(blackpiece.getXC(), blackpiece.getYC(), blackpiece.getName(), newWhite.getXC(), newWhite.getYC(), currentWhite.getXC(), currentWhite.getYC());
                values.push(value);

              }

              else if(name.contains("Bishop")){
                value = getBishopPoints(blackpiece.getXC(), blackpiece.getYC(), blackpiece.getName(), newWhite.getXC(), newWhite.getYC(), currentWhite.getXC(), currentWhite.getYC());
                values.push(value);

            }

            else if(name.contains("Knight")){
              value = getKnightPoints(blackpiece.getXC(), blackpiece.getYC(), blackpiece.getName(), newWhite.getXC(), newWhite.getYC(), currentWhite.getXC(), currentWhite.getYC());
              values.push(value);
            }

            else if(name.contains("Queen")){
              value = getQueenPoints(blackpiece.getXC(), blackpiece.getYC(), blackpiece.getName(), newWhite.getXC(), newWhite.getYC(), currentWhite.getXC(), currentWhite.getYC());
              values.push(value);

            }

            else if(name.contains("King")){
              value = getBlackKingPoints(blackpiece.getXC(), blackpiece.getYC(), blackpiece.getName(), newWhite.getXC(), newWhite.getYC(), currentWhite.getXC(), currentWhite.getYC());
              values.push(value);
            }
          ...
          //we now have a highest value for each piece 6
          //only take the highest value
          while(!values.empty()){
            int current = (int)values.pop();
            if(current > tempHigh){
              tempHigh = current;
            }
          }
          highVales.push(tempHigh);
          // have two stacks that are side by side
          //the top of the point stack,which is the highest value blck can take
          //is corresponding move the white is looking at.


        }
        System.out.println("Black Values  : "+highVales.size());
        System.out.println("White Moves  : "+ whiteMoves.size());
        //we need to exclude checking the black piece we are going to take IF we take one e.g. if piecePresent()
        //the points stack should be the SAME ORDER AND SIZE of the "testing" STACK.
        selectedMove = agent.twoLevelsDeep(whiteMoves,highVales);
      }
